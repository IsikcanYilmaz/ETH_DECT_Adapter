...
/*
 * YANKED FROM https://github.com/lauri-piikivi/dect-nr-samples.git
 */
...
#include "mac_main.h"
#include "../lean_wiznet_driver/lean_wiznet_driver.h"

LOG_MODULE_REGISTER(mac_main, LOG_LEVEL_WRN);

static K_MUTEX_DEFINE(app_mutex);
static K_MUTEX_DEFINE(dongle_mutex);
...
{
  struct app_event evt = {.type = APP_EVT_TX_AVAILABLE};
  app_event_put(&evt);
}
...
static int configure_ft(void)
{
  ...
  err = dect_adapter_cluster_configure_ft(current_carrier, ft_period_ms, CONFIG_APP_NETWORK_ID, CONFIG_APP_TX_POWER,ft_rach_fill_percentage);
  if (err != 0) {
...
static int send_ascii(enum app_mode source_mode, const char *text)
{
  ...
  LOG_INF("SEND MESSAGE: target_rd=%u len=%zu data=%s\n", target_long_rd_id, strlen(tx_buf) + 1, tx_buf);
  pending_tx_transaction_id = tx_transaction_id++;

  err = dect_adapter_dlc_data_send(pending_tx_transaction_id, APP_FLOW_ID, target_long_rd_id, tx_buf, strlen(tx_buf) + 1);
  if (err != 0) {
    return err;
  }

  LOG_INF("SEND queued: tx=%u\n", pending_tx_transaction_id);
  return 0;
}

int dect_send(enum app_mode source_mode, const char *buf, size_t len)
{
  int err;
  uint32_t target_long_rd_id;
  if (source_mode == APP_MODE_FT) 
  ...
  else if (source_mode == APP_MODE_PT)
  {
    if (!pt_associated) 
    {
      return -ENOTCONN;
  ...
  {
    LOG_ERR("%s Bad mode! %d", __FUNCTION__, source_mode);
    return 1;
  }
  ...
  if (err != 0) 
  {
    ...
    if (err == -ENOMEM)
    {
      LOG_ERR("No memory. %d byte packet", len);
    }
    return err;
  }

  LOG_INF("SEND: target_rd=%u len=%zu\n", target_long_rd_id, len);
  LOG_INF("SEND queued: tx=%u\n", pending_tx_transaction_id);
  return 0;
}

static int dect_receive(const char *buf, size_t len)
{
  // TODO
}
...
static void process_dlc_rx_event(const struct app_event *evt)
{
  ...
  // ring_buf_put(&dect_rx_ringbuf, evt->dlc_rx.text, evt->dlc_rx.len);
  ...
  if (status == 0) {
    ...
    if (status == 8) {
      ...
      if (pt_association_retries >= 10) {
        ...
        uint32_t retry_ms = (pt_parent_ft_period_ms > 0) ? 5 * pt_parent_ft_period_ms : 500;
        LOG_WRN("PT association NO_RESPONSE (status=8) rd=%u, retry %u/10 in %u ms", long_rd_id, pt_association_retries, retry_ms);
        k_work_reschedule(&pt_associate_work, K_MSEC(retry_ms));
      }
    } else {
      LOG_ERR("Association failed: status=%d rd=%u — triggering recovery",status, long_rd_id);
      pt_association_retries = 0;
...
// JON this fn looks at $ethRxQueue (received FROM ethernet queue)
// and sends them thru DECT
static void process_dect_handle_tx(const struct app_event *evt)
{
  LOG_INF("JON TX %s", __FUNCTION__);
  if (k_queue_is_empty(&ethRxQueue))
  {
    LOG_ERR("ETH RX queue empty");
    return;
  }
...
static void process_app_event(const struct app_event *evt)
{
  switch (evt->type) {
    ...
    case APP_EVT_NTF_ASSOCIATION:
      ...
      break;
    /* JON */
    case APP_EVT_TX_AVAILABLE:
      process_dect_handle_tx(evt);
      break;
    default:
      break;
  }
...
////// DONGLE ////// TODO Make these prettier / move elsewhere
// dongle stuff
// static void dongle_work_handler(struct k_work *work);
// static K_WORK_DELAYABLE_DEFINE(dongle_work_ptr, dongle_work_handler);
// #define DONGLE_POLL_PERIOD_US 100
// #define DECT_RING_BUF_SIZE 10000
// static volatile bool dongle_enabled = false;
// RING_BUF_DECLARE(dect_rx_ringbuf, DECT_RING_BUF_SIZE);
//WORK HANDLERS
// static void dongle_work_handler_serial(struct k_work *work)
// {
//   struct k_work_delayable *dwork = k_work_delayable_from_work(work);
//   size_t write_len = 0;
//   if (current_mode == APP_MODE_PT || current_mode == APP_MODE_FT)
//   {
//     char fromPicoBuf[256];
//     char fromDectBuf[256];
//
//     k_mutex_lock(&dongle_mutex, K_FOREVER);
//
//     // FROM PICO TO US. NEEDS TO BE SENT OTA
//     if (serial_rx_available()) 
//     {
//       size_t serial_read_len = serial_read(fromPicoBuf, APP_DATA_LEN_MAX);
//       LOG_INF("%d bytes serial_read", serial_read_len);
//       dect_send(current_mode, fromPicoBuf, serial_read_len);
//     }
//
//     // FROM OTHER DECT TO US. NEEDS TO BE SEND THRU UART
//     size_t dect_rx_len = ring_buf_size_get(&dect_rx_ringbuf);
//     if (dect_rx_len)
//     {
//       size_t dect_read_len = ring_buf_get(&dect_rx_ringbuf, fromDectBuf, 256);
//       write_len = serial_write(fromDectBuf, dect_read_len);
//       LOG_INF("%d bytes dect_read. wrote %d bytes to uart", dect_rx_len, write_len);
//     }
//
//     k_mutex_unlock(&dongle_mutex);
//
//   }
//   else 
//   {
//     LOG_ERR("App mode not set! Run dongle start to kick off your dongle");
//     return 1;
//   }
//
//   if (dongle_enabled)
//   {
//     k_work_reschedule(dwork, K_USEC((write_len + 1) * 10)); 
//   }
// }

// static void dongle_thread(void)
// {
//   LOG_INF("Dongle thread starting");
//   while(true)
//   {
//     LOG_INF("Waiting for dongle start");
//     while(!dongle_enabled)
//     {}
//
//     while(dongle_enabled)
//     {
...
static void cb_ntf_dlc_data_rx(uint32_t long_rd_id, const void *data, size_t data_len)
{
  ...
  LOG_INF("DLC RX ntf: rd=%u len=%zu data=%s\n", long_rd_id, data_len, evt.dlc_rx.text);
  app_event_put(&evt);
}
...
static int cmd_send(const struct shell *shell, size_t argc, char **argv)
{
  ...
  for (size_t i = 1; i < argc; i++) {
    int written = snprintk(text + pos, sizeof(text) - pos, "%s%s", i > 1 ? " " : "", argv[i]);
  ...
  if (source_mode == APP_MODE_PT) {
    if (err == 0) {
      ...
      if (pt_dlc_tx_fail_count >= 3) {
        LOG_ERR("PT: DLC TX failed %u times — triggering recovery\n",
               pt_dlc_tx_fail_count);
        pt_dlc_tx_fail_count = 0;
        pt_associated = false;
...
static int cmd_period(const struct shell *shell, size_t argc, char **argv)
{
  ...
  const int periodLL = 10;
  const int periodUL = 32000;

  ARG_UNUSED(argc);

  if (value < periodLL || value > periodUL) {
    shell_error(shell, "PERIOD must be between 10 and 32000 ms");
    return -EINVAL;
  }
...
static int cmd_pt(const struct shell *shell, size_t argc, char **argv)
{
  ...
  if (modem_synced) 
  {
    ...
    shell_print(shell, "Subscribing to cluster beacon on ch=%u — will associate with rd=%u when RA schedule is known", channel, rd_id);
  } 
  else 
  {
...
static int cmd_help_dect(const struct shell *shell, size_t argc, char **argv)
{
  ...
  shell_print(shell, "  PERIOD <ms>             Set FT cluster beacon period (10..32000 ms)");
...
static int cmd_dongle(const struct shell *shell, size_t argc, char **argv)
{

  if (current_mode == APP_MODE_IDLE)
  {
    shell_print(shell, "First do FT or PT");
    return 1;
  }

  if (strncmp(argv[1], "start", 5) == 0)
  {
    // if (!dongle_enabled)  // enable our dongle
    // {
      // k_work_init_delayable(&dongle_work_ptr, dongle_work_handler);
      // k_work_schedule(&dongle_work_ptr, K_NO_WAIT);
      // dongle_enabled = true;
      shell_print(shell, "Dongle enabled");
    // }
  }
  else if (strncmp(argv[1], "stop", 4) == 0)
  {
    // if (dongle_enabled) // disable our dongle
    // {
      // serial_deinit();
      // LOG_INF("Serial deinitialized");
      // dongle_enabled = false;
      shell_print(shell, "Dongle disabled");
    // }
  }
  else
  {
    shell_error(shell, "Bad argument. Must be <start|stop>");
    return 1;
  }
}

static int cmd_bigsend(const struct shell *shell, size_t argc, char **argv)
{
  int numPayloadBytes = atoi(argv[1]);
  LOG_INF("Sending payload with %d bytes", numPayloadBytes);

  char buf[numPayloadBytes];
  for (int i = 0; i < numPayloadBytes; i++)
  {
    buf[i] = (char) ('a'+i);
  }
  dect_send(current_mode, buf, numPayloadBytes);
...
SHELL_CMD_ARG_REGISTER(DONGLE, NULL, "Dongle <start|stop>", cmd_dongle, 2, 0);
SHELL_CMD_ARG_REGISTER(dongle, NULL, "Dongle <start|stop>", cmd_dongle, 2, 0);
...
{
  return (current_mode == APP_MODE_PT || current_mode == APP_MODE_FT);
}
